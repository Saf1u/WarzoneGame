...
/**
 * The play method that would issue an order , put in the player list of orders and remove
 * the card from the hand
 */
Card* Card::play(Player x) {

     cout <<"Inside Play Method" <<endl;
     // a définir par  après
     //1 Creates an order Object an place it in the palyer list
     Order order;

    //2 Place the order in the player list of orders
    // x.IssueOrder(order);

     //3Removes the card from the hand and places it in the Deck
     //this is pointer

     cout << "We are removing " <<endl;
     cout << *this << endl;
    cout << "We are removing " <<endl;
     x.getHand()->remove(this);
...
void Hand::Add(Card c){
    Card * cpointer =  new Card (c);

    _collection.push_back(cpointer);
}
...
Deck::Deck(){
   ...
   // _MyHand =  new vector<Card>(*(vector<Card> ))   ;
...
Deck::Deck(vector<Card*>  deck) {
    ...
    for (Card* i: (deck) ) {
        ...
        if(type == "Reinforcement") {
            //cout << "It is a Reinforcement" << endl;
            Reinforcement a = Reinforcement(type) ;
            Card * T1 = new Card (a);
            this->_deck.push_back(T1);
        }
