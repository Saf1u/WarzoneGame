...
Territory::Territory(string name, int num,int numberOfArmies,int continent) {
    ...
    this->maxNumberOfArmies=0;
    // deleted bonus note bc it was too big but might be a problem
    this->continentNumber=continent;

}
int Territory::get_numberOfArmies() {
...
void Territory::verifyTerritory() {
    if(this->numberOfArmies<0){

        this->numberOfArmies=0;
    }
    if(this->numberOfArmies>maxNumberOfArmies){

        numberOfArmies=maxNumberOfArmies;
    }
}

void Map::continentBonus() {
    vector<Territory*> continents = this->continents;
    int index = 1;
    for (Territory* t: continents) {
        t->Bonus = index;
        index++;
    }
}
...
//aS WE ARE UNABLE TO ACCESS THE MAP CLASS FROM WITHIN THE TERRITORY CLASS, THIS HELPER FUNCTION ALLOWS A RECURSIVE COPY
// OF all possible territories by executing a sort of dfs starting from the passed in territory
...
//Map copy constructor that copies a whole map by first copying a single territory (thereby copying a whole map) and
// then adding the copied territories to the respective continents (without
...
ostream& operator <<(ostream &strm,const Territory&t) {
    return strm<< "The Territory Name is "<<t.name<<","
    << "The Territory number is "
    <<t.number<<", and the Number of Armies it contains is : "<<t.numberOfArmies<<endl;
}
...
ostream& operator <<(ostream &strm,const Map&t) {
    ...
    for (i=0;i<t.map.size();i++){

       strm <<"territory is:"<<*(t.map[i])<<endl;
        strm<<"its Neighbors are:"<<endl;
        vector<Territory*> tempb=(t.map[i])->edges;
        for(j=0;j<tempb.size();j++){

            strm<<"Neighbor of:"<<*(t.map[i])<<endl;
            strm<<*(tempb[j])<<endl;
        }
        strm << endl;
    };
